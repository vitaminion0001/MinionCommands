...
class Cooldowns {
    // perUser:
    // <`${userId}-${qactionId}`: expires>
    // peruserPerGuild:
    // <`${iserId}-${guildId}-${actionId}`: expires>
    // perGuild:
    ...
    constructor({
        ...
        dbRequired = 150 // 2.5 minutes
    }) {
        this._instance = instance
        this._errorMessage = errorMessage
        this._botOwnerBypass = botOwnersBypass
    ...
    async updateCooldown(cooldownUsage, expires) {
        ...
        if (secondsDiff > this._dbRequired) {
            await cooldownSchema.findByIdAndUpdate(
                {
                    _id: key
                },
                {
    ...
    canBypass(userId) {
        return this._botOwnerBypass && this._instance.botOwners.includes(userId)
    }
